...
/**
 * Calls a plugin's initialize method, passing in this as an argument. On disconnect,
 * the plugin's cleanup method will be invoked.
 * @param {Plugin} plugin a plugin to load. Must provide initialize(xtb) and cleanup(xtb) methods.
 */
ExtBot.prototype.loadPlugin = function(plugin) {
 ...
 * Calls the cleanup() method of any plugins that have been loaded.
 */
ExtBot.prototype.disconnect = function() {
    this.rlq.close();
    // call each plugin's cleanup hook
    this.pluginsLoaded.forEach(plugin => plugin.cleanup(this));
